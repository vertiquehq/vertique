...
/**
 * Continuous, fractional token refill: progress toward the next token accrues every instant
 * rather than arriving all at once at the period boundary (contracts/rate-limit-runtime.md,
 * "Bucket4j translation contract" — maps to Bucket4j's {@code refillGreedy}).
 *
 ...
 */
public record GreedyRateLimitRefill(long tokens, Duration period) implements RateLimitRefill {
     ...
     */
    @JsonCreator
    static GreedyRateLimitRefill fromJson(
            @JsonProperty("tokens") long tokens, @JsonProperty("periodMs") long periodMs) {
        return new GreedyRateLimitRefill(tokens, Duration.ofMillis(periodMs));
    }
}
