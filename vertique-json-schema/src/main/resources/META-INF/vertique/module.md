...
- **The floor.** A field or getter with a Victools member scope is described by Victools' own Jackson
  ...
  wire name like every other creator (a non-canonical creator parameter whose wire name matches no
  component name therefore joins nothing: it is neither published nor reserved, the same posture an
  ordinary class's renamed creator parameter has). This is what keeps a `@JsonCreator` static-factory
  parameter's own constraint from being
