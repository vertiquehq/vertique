...
Each layer's chain is appended **exactly as declared** — same order, repeats included — minus the processor classes an enclosing level already contributed. So `@Sanitize(StripHtml.class)` on a type and `@Sanitize({DecodeEntities.class, StripHtml.class})` on one of its fields compose to `StripHtml, DecodeEntities, StripHtml`: the field's declared decode-then-strip order is what runs. Order is preserved rather than deduplicated because idempotence is not commutativity — `Canonicalizer` and `Sanitizer` require the former and say nothing about the latter, so collapsing the repeat would silently invert a deliberately ordered pipeline.

Dropping classes an enclosing level already contributed is what keeps the chain bounded. Policies are resolved per type, so a type's own chain is offered again at every level of a recursive graph; because a class already inherited is never appended again, the composed chain grows only while *new* declaration sites are reached. Its length is therefore bounded by the application's declared type graph and is independent of how deeply the request body nests.
