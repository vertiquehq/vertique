...
Concretely: a `String` field's raw declared class is not prepared — a wire fragment that disagrees with the declared `String` shape dispatches against it, but `String`'s own metadata declares no fields, so the dispatch is schema-free. Neither is a raw-container entry point or field whose element schema cannot be determined — the engine falls back to dispatching against the raw container class only because there is nothing more specific to key on, and that raw class's own metadata is equally empty. A `Map`, an enum, or `Object` reached through an *annotated* field, by contrast, is still prepared: the field genuinely is dispatched against that declared type whenever the wire value is a Map, so a wire-name collision there is still worth catching at registration rather than on the first request. And a collection subtype's own raw container class (as opposed to its element type) is still prepared too, for the same reason — the engine dispatches against it on a wire/declared shape mismatch, which is a real, if rarer, code path.
