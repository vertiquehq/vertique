...
| `@JsonUnwrapped` members | Reported through `promotedFields(Class)` and `promotedField(Class, String)`, keyed by the **wire** name the mapper binds — a promoted key is never added to the projection, so it can never shadow a same-named field of the enclosing type. A `prefix` or `suffix` is applied through Jackson's own `NameTransformer`, nested unwrapping chains them, and an inner field's `@JsonAlias` is promoted alongside its primary name. Members reached through a getter or setter are promoted like field-backed ones. Two promoted members claiming one wire key fail composition |
