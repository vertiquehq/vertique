...
| `@JsonUnwrapped` members | Reported through `promotedFields(Class)`: the inner type's fields arrive as keys of the enclosing object, so the projection resolves each promoted key and names the type it is bound into, and the engine applies that field's own declared policies. A `prefix` or `suffix` is applied through Jackson's own `NameTransformer`, and nested unwrapping chains them. Two promoted keys that would collide on one logical name fail composition |
