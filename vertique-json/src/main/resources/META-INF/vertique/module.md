...
| `@JsonUnwrapped` members | Reported through `promotedFields(Class)` and `promotedField(Class, String)`, keyed by the **wire** name the mapper binds — a promoted key is never added to the projection, so it can never shadow a same-named field of the enclosing type. A `prefix` or `suffix` is applied through Jackson's own `NameTransformer` and nested unwrapping chains them. Members reached through a getter or setter are promoted like field-backed ones. Claims on one key resolve as Jackson binds them: a bean's own property claims a key before anything unwrapped into it, at every depth, and within one level a primary name claims before an alias. An inner field's `@JsonAlias` is promoted through a bare unwrap only — Jackson does not apply a prefix or suffix to aliases. Two members at one depth claiming one key fail composition naming the owner being composed, because Jackson feeds both from the same value |
| Bound Java names | `boundJavaNames(Class)` returns the internal name of every property the mapper's deserialization introspection reports — the names Jackson derives from the fields, accessors and creator parameters it finds. A field reached only through an accessor of a different implicit name is not among them, nor is an ignored or transient field, which is what lets the engine refuse at startup a declared policy the mapper would never reach. It is `null` for a type the mapper binds outside its declaration view — a custom deserializer, a builder, or a delegating creator — so such a type is trusted rather than refused. That trust is a documented limitation for builders: a governed field a hand-written builder writes under a differently named method is not detected, and its policy silently never runs. Lombok's `@Builder @Jacksonized` names its methods after the fields and is unaffected; name builder methods after the fields they write. The detection reads the class's own `@JsonDeserialize(using = …)` only: a deserializer registered through a module or declared on the referencing field leaves the type enumerated from a declaration view Jackson does not use, so a governed field named differently from what the introspection reports is refused at startup rather than trusted. Declare the deserializer on the class `unroutableWireNames(Class)` names the keys bound only through a creator parameter with no linked field or setter and no declared field of the parameter's own name: `@JsonCreator Dto(@JsonProperty("street_name") String s)` binds `street_name` into a field Jackson cannot name. Name the parameter after the field (`@JsonProperty("streetName")`), or give the field the same `@JsonProperty` name so Jackson links the two under the field's own name. Record components are always linked. A key the owner ignores (`@JsonIgnoreProperties`) is never promoted, because Jackson drops it before any unwrapped member sees it |
