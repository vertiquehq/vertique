...
/**
 * Verifies the last-wins fold semantics for {@link ServiceExceptionMapperCustomizer} when sorted by
 ...
 * </ol>
 */
class ServiceExceptionMapperLastWinsTest {

    /**
     * Minimal {@link ServiceExceptionMapperCustomizer} test double that records its name into a
     * shared list when {@link #customize} is called, instead of touching a real mapper.
     */
    private static final class RecordingCustomizer implements ServiceExceptionMapperCustomizer {
    ...
    void higherPriorityWins() {
        ...
        List<ServiceExceptionMapperCustomizer> customizers = new ArrayList<>(List.of(high, low));
    ...
    void systemLastAppliesLastAndWins() {
        ...
        List<ServiceExceptionMapperCustomizer> customizers = new ArrayList<>(List.of(systemLast, app));
        customizers.sort(OrderedExtension.comparator());
        customizers.forEach(c -> c.customize(null));
