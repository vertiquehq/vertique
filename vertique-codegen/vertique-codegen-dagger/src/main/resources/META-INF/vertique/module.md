...
**`@Path` (JAX-RS) resource binding is no longer owned by this processor.** As of CG-010, `@Path`-resource DI binding (`@Provides @IntoSet @JaxRsResources Object`) is emitted by `vertique-codegen-jaxrs`'s `JaxRsPipelineProcessor`, which also handles discovery, validation, and runtime optimization for JAX-RS resources. Users who need JAX-RS resource wiring should add `vertique-codegen-jaxrs` to `annotationProcessorPaths`; `vertique-codegen-dagger` is not required for that qualifier. See `dev.vertique:vertique-codegen-jaxrs`.

`@ServiceContract` impl wiring is owned by `vertique-codegen-services` (CG-005), not by this processor. See `dev.vertique:vertique-codegen-services` for the CG-005 adoption recipe.
...
### Step 1 — add the processor to `annotationProcessorPaths`

If the parent POM already configures `<annotationProcessorPaths>` (e.g., for Dagger and Lombok), use `combine.children="append"` to extend rather than replace the inherited list:
...
Without `combine.children="append"`, the child POM's `<annotationProcessorPaths>` replaces the parent's, dropping Dagger and Lombok — the build fails.
...
@Component(modules = {
    ...
    GeneratedJaxRsResourcesModule.class,   // generated by vertique-codegen-jaxrs (CG-010)
    GeneratedDelayedJobsModule.class,      // generated by vertique-codegen-dagger
})
...
### `combine.children="append"` is required for child POMs

Any child POM that inherits `<annotationProcessorPaths>` from a parent and adds `vertique-codegen-dagger` **must** use `combine.children="append"`. Without it, Maven replaces the inherited list, dropping Dagger (`dagger-compiler`) and Lombok, which breaks `@Component` generation and `@Slf4j`/`@RequiredArgsConstructor` expansion.
