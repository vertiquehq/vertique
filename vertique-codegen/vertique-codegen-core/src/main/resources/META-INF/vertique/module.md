...
The caller (`AopProxyEmitter`) is responsible for materializing each `AnnotationLiteralRef` — emitting (and per-compilation deduplicating) the `<Ann>$Literal` class via `AnnotationLiteralEmitter` and rejecting unsupported attribute kinds before construction. The same bounded-attribute-kind gate guards both method-level and parameter-level literals: an unsupported attribute kind is a compile error, not a silent reflective fallback.
