...
// The REST starter is mechanism-neutral: no JWT artifact from any group may reach the classpath
// (dev.vertique:vertique-rest-auth-jwt, io.vertx:vertx-auth-jwt, com.nimbusds:nimbus-jose-jwt, …).
List<String> jwtOnClasspath = classpathCoordinates.findAll { it.toLowerCase().contains("jwt") }.toList()
assert jwtOnClasspath.isEmpty():
        "REST starter leaks JWT artifacts onto the compile/runtime classpath: ${jwtOnClasspath}"
