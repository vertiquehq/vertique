...
- ADR-0121: `openapi.json` is documentation-only; binding is separate from validation — establishes that `openapi.json` is a documentation artifact only; request binding and validation are driven by annotation-synthesized JSON Schema, not the OpenAPI spec.
