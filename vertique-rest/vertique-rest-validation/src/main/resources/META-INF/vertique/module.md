...
- ADR-0119: Security-Scheme Handler Decoupling from RouterBuilder — establishes plain-router registration and the route handler surfaces used by validation gates.
- ADR-0120: Annotation-Synthesized, Strategy-Pluggable Validation — governs the `RequestValidationStrategy` SPI, the operation-schema seam, and the `web-validation` / `openapi-contract` / `none` built-in set.
- ADR-0121: `openapi.json` is documentation-only; binding is separate from validation — establishes that `openapi.json` is a documentation artifact only on the default `web-validation` path this module implements; request binding and validation here are driven by annotation-synthesized JSON Schema, not the OpenAPI spec. The opt-in `openapi-contract` strategy (`vertique-rest-openapi-validation`) does load `openapi.json` at runtime and validates requests against it instead.
- ADR-0142: Shared Parameter Conversion SPI — establishes the single `ParamConversionResolver` that `WebValidationStrategy` injects and threads into its `DefaultBoundRequest`, alongside the `rest-jaxrs` dispatch path and the `rest-openapi-validation` strategy.
- ADR-0179: File-Part Validation and Content Verifier — governs `@FilePart`, `FileContentVerifier`, strategy scoping, post-spool limits, and temporary-file lifetime.
