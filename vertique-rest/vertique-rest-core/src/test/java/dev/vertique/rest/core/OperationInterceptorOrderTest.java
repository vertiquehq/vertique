...
/**
 * Verifies that {@link OperationInterceptor} participates in the {@link OrderedExtension} ordering
 ...
 */
class OperationInterceptorOrderTest {

    /**
     * Minimal {@link OperationInterceptor} test double with configurable phase and priority.
     * All callback methods retain their default no-op / pass-through implementations;
     * only ordering behaviour is under test.
     */
    private static final class TestInterceptor implements OperationInterceptor {
    ...
    void phaseDominatesPriority() {
        ...
        List<OperationInterceptor> interceptors = new ArrayList<>(List.of(application, systemFirst));
    ...
    void lowerPriorityFirst() {
        ...
        List<OperationInterceptor> interceptors = new ArrayList<>(List.of(high, low));
        interceptors.sort(OrderedExtension.comparator());
