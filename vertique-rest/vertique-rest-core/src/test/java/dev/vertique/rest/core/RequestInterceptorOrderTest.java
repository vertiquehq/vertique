...
/**
 * Verifies that {@link RequestInterceptor} participates in the {@link OrderedExtension} ordering
 ...
 */
class RequestInterceptorOrderTest {

    /**
     * Minimal {@link RequestInterceptor} test double with configurable phase and priority.
     * All callback methods retain their default no-op / pass-through implementations;
     * only ordering behaviour is under test.
     */
    private static final class TestInterceptor implements RequestInterceptor {
    ...
    void phaseDominatesPriority() {
        ...
        List<RequestInterceptor> interceptors = new ArrayList<>(List.of(application, systemFirst));
    ...
    void lowerPriorityFirst() {
        ...
        List<RequestInterceptor> interceptors = new ArrayList<>(List.of(high, low));
        interceptors.sort(OrderedExtension.comparator());
