...
3. For each public method with an HTTP method annotation (`@GET`, `@POST`, `@PUT`, `@DELETE`, `@PATCH`, `@HEAD`, `@OPTIONS`):
   ...
   - **Startup validation** (all violations collected and thrown as `RouteRegistrationException` after all resources are scanned):
     ...
     - `NON_COMPARABLE_SORTED_SET_ELEMENT`: a parameter declared as `SortedSet<T>`/`NavigableSet<T>` has an element type that is not comparable to itself — it does not implement `Comparable` at all, its effective `compareTo` accepts a type that `T` is not assignable to, or it only *declares* such a `Comparable` (an interface, a sealed interface, or an abstract class that leaves `compareTo` abstract) so that its implementors compare against that other type. Both shapes are materialized as a `TreeSet` (natural ordering) and a JAX-RS declaration cannot supply a `Comparator`, so the shape has no valid materialization and is rejected instead of throwing `ClassCastException` per request; declare `Set<T>`/`List<T>`/`Collection<T>` instead, or make the element type `Comparable<T>`. Scoped to `@QueryParam`/`@HeaderParam`/`@CookieParam`/`@FormParam` — a `SortedSet<T>` *entity body* is deserialized by the selected `RequestBodyDecoder`, so its element ordering is that decoder's contract, not route validation's (the built-in JSON decoder does resolve `SortedSet` to a `TreeSet`, so a non-self-comparable body element fails per request under it)
...
- **Sorted shapes need self-comparable elements** — `SortedSet<T>`/`NavigableSet<T>` are materialized as a `TreeSet`, so `T` must be comparable to itself. What decides is the type the synthesized `compareTo(Object)` bridge casts to before delegating: the shape is accepted when `T` is assignable to it. That type is read from two pieces of evidence together — the erasure of `Comparable`'s type argument where `T`'s hierarchy *declares* it, and the parameter type of the *effective* (non-bridge) `compareTo` that `T` actually inherits. The declaration alone cannot see a type variable bound further down; the inherited method alone does not exist for a type that only declares `Comparable`. Where several `compareTo` overloads are visible, the declared type argument identifies the real one, so the verdict never depends on reflection's unspecified method order.
  - **Accepted** — `String`, the boxed numerics, `Boolean`, `Character`, enums, a raw `implements Comparable`, `Comparable<S>` for a supertype `S` of `T` (e.g. `LocalDateTime`'s `Comparable<ChronoLocalDateTime<?>>`), a forwarding interface bound to the self type (`class X implements Ord<X>` over `interface Ord<T> extends Comparable<T>`), the self-bounded idiom (`class Node<T extends Node<T>> implements Comparable<T>`), a subclass of an *unbounded* `Base<T> implements Comparable<T>` (a type variable erases to its leftmost bound, so these really do compile to a `compareTo` that accepts the element type), and an interface or abstract element type comparable to itself (`interface X extends Comparable<X>`).
  - **Rejected** with `NON_COMPARABLE_SORTED_SET_ELEMENT` (see [Startup validation](#jaxrsrouteregistrar)), instead of throwing `ClassCastException` per request — an element type that does not implement `Comparable` at all; one whose effective `compareTo` takes an unrelated type (`class Money implements Comparable<BigDecimal>`, `class Bad implements Ord<String>`, a subclass of a *bounded* `Base<T extends CharSequence> implements Comparable<T>`); and an interface, sealed interface, or abstract class that *declares* `Comparable<Unrelated>` without implementing it (`interface Bad extends Comparable<String>`) — its implementors compare against that other type, so a `TreeSet` of them fails just the same.
