...
- On the **codegen** path, both `ExecutionPlanEmitter` and `JaxRsDescriptorEmitter` back the composed view with a compile-time literal-backed `ParameterMetadata` implementation (one standalone generated class per parameter, mirroring `vertique-codegen-aop`'s `MetadataEmitter`-generated metadata), materializing each `@Retention(RUNTIME)` parameter annotation into a `<Ann>$Literal` constant with no reflection at request time or class-registration time.
