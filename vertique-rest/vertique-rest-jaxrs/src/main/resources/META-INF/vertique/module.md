...
- **Sorted shapes need self-comparable elements** — `SortedSet<T>`/`NavigableSet<T>` are materialized as a `TreeSet`, so `T` must be comparable to itself. What decides is the parameter type of `T`'s *effective* (non-bridge) `compareTo` method — precisely what the synthesized `compareTo(Object)` bridge casts to before delegating: the shape is accepted when `T` is assignable to that parameter type. `String`, the boxed numerics, `Boolean`, `Character`, enums, a raw `implements Comparable`, `Comparable<S>` for a supertype `S` of `T` (e.g. `LocalDateTime`'s `Comparable<ChronoLocalDateTime<?>>`), a forwarding interface bound to the self type (`class X implements Ord<X>` over `interface Ord<T> extends Comparable<T>`), the self-bounded idiom (`class Node<T extends Node<T>> implements Comparable<T>`), and a subclass of an *unbounded* `Base<T> implements Comparable<T>` all qualify — a type variable erases to its leftmost bound, so these really do compile to a `compareTo` that accepts the element type. An element type that does not implement `Comparable` at all, or whose effective `compareTo` takes an unrelated type — `class Money implements Comparable<BigDecimal>`, `class Bad implements Ord<String>`, or a subclass of a *bounded* `Base<T extends CharSequence> implements Comparable<T>` — fails route registration with `NON_COMPARABLE_SORTED_SET_ELEMENT` (see [Startup validation](#jaxrsrouteregistrar)) rather than throwing `ClassCastException` per request.
