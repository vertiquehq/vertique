...
3. For each public method with an HTTP method annotation (`@GET`, `@POST`, `@PUT`, `@DELETE`, `@PATCH`, `@HEAD`, `@OPTIONS`):
   ...
   - **Startup validation** (all violations collected and thrown as `RouteRegistrationException` after all resources are scanned):
     ...
     - `NON_COMPARABLE_SORTED_SET_ELEMENT`: a parameter declared as `SortedSet<T>`/`NavigableSet<T>` has an element type that does not implement `Comparable` — both shapes are materialized as a `TreeSet` (natural ordering) and a JAX-RS declaration cannot supply a `Comparator`, so the shape has no valid materialization and is rejected instead of throwing `ClassCastException` per request; declare `Set<T>`/`List<T>`/`Collection<T>` instead, or make the element type `Comparable`
...
- **Sorted shapes need `Comparable` elements** — `SortedSet<T>`/`NavigableSet<T>` are materialized as a `TreeSet`, so `T` must implement `Comparable`. A non-`Comparable` element type fails route registration with `NON_COMPARABLE_SORTED_SET_ELEMENT` (see [Startup validation](#jaxrsrouteregistrar)) rather than throwing `ClassCastException` per request; `String`, the boxed numerics, `Boolean`, `Character`, and enums all qualify.
