...
3. For each public method with an HTTP method annotation (`@GET`, `@POST`, `@PUT`, `@DELETE`, `@PATCH`, `@HEAD`, `@OPTIONS`):
   ...
   - **Startup validation** (all violations collected and thrown as `RouteRegistrationException` after all resources are scanned):
     ...
     - `NON_COMPARABLE_SORTED_SET_ELEMENT`: a parameter declared as `SortedSet<T>`/`NavigableSet<T>` has an element type that is not comparable to itself — it does not implement `Comparable` at all, or implements `Comparable<X>` for a type `X` that is neither `T` nor a supertype of it. Both shapes are materialized as a `TreeSet` (natural ordering) and a JAX-RS declaration cannot supply a `Comparator`, so the shape has no valid materialization and is rejected instead of throwing `ClassCastException` per request; declare `Set<T>`/`List<T>`/`Collection<T>` instead, or make the element type `Comparable<T>`
...
- **Sorted shapes need self-comparable elements** — `SortedSet<T>`/`NavigableSet<T>` are materialized as a `TreeSet`, so `T` must be comparable to itself: it must implement `Comparable<T>`, or `Comparable<S>` for some supertype `S` of `T`. `String`, the boxed numerics, `Boolean`, `Character`, and enums all qualify. An element type that does not implement `Comparable` at all, or implements it against an unrelated type (`class Money implements Comparable<BigDecimal>` — whose synthesized `compareTo(Object)` bridge casts and throws), fails route registration with `NON_COMPARABLE_SORTED_SET_ELEMENT` (see [Startup validation](#jaxrsrouteregistrar)) rather than throwing `ClassCastException` per request. A `Comparable` declaration whose type argument is not a concrete class — a raw `implements Comparable`, or a self-referential generic such as an enum's inherited `Comparable<E>` — is accepted.
