...
| A framework-level element **ordering guarantee** for multi-value params | Neither Vert.x nor Jakarta REST documents one (F8, F12); promising one would pin one version's observed behavior | Issue [#60](https://github.com/vertiquehq/vertique-dev/issues/60): "Decide whether multi-value param ordering is a framework guarantee" |
| Native multipart in non-`List` collection shapes (`Set<FileUpload>` …) | **Not deferred silently** — now rejected loudly at startup (§4 decision 6). Promoting it to supported would need native materialization, `@FilePart` validation, and security tests | Issue [#61](https://github.com/vertiquehq/vertique-dev/issues/61): "Decide whether non-List multipart collection shapes become supported" |
...
| **No compile-time mirror for the shape rules** — neither `UNSUPPORTED_MULTIPART_COLLECTION_SHAPE` nor `DUPLICATE_PARAM_NAME_MULTIPLICITY_CONFLICT` has a processor-side diagnostic | Both are runtime `RouteValidator` checks; the codegen module has no mirror for either. Adding one is a separate slice covering both together, not half of one | Issue [#69](https://github.com/vertiquehq/vertique-dev/issues/69): "Mirror the FORM shape rules as compile-time diagnostics" |
| **Inner class of a parameterized owner** — `List<Outer<String>.Inner>` is a `ParameterizedType` reflectively (empty args, parameterized owner) so the reflective scanner resolves no element type, while the codegen mirror accepts it | Near-zero reachability: a non-static member class of a generic outer, used as a String-convertible collection element. Two-line fix plus matrix rows if it ever matters | Issue [#71](https://github.com/vertiquehq/vertique-dev/issues/71): "Mirror inner-classes-of-parameterized-owners in the codegen element-type gate" |
| **RFC 6265 cookie names are case-sensitive** while the framework now matches them case-insensitively on both halves, so `@CookieParam("session")` is satisfiable by a wire cookie named `Session`, and two cookies differing only in case collapse (last wins) | Pre-existing in the map keying; the security fix only made the descriptor half agree with it, which is what was actually broken. Changing the semantic is a separate decision | Issue [#72](https://github.com/vertiquehq/vertique-dev/issues/72): "Decide whether cookie name matching should be case-sensitive per RFC 6265" |
| **`raw.toString()` double-conversion** in the multiplicity fallback, lossy for a converter whose round-trip is not `toString`-symmetric | Its only legal trigger was the duplicate-name declaration now rejected at startup; the remaining triggers are a custom `BoundRequest`, a codegen divergence, or path collection support | Issue [#73](https://github.com/vertiquehq/vertique-dev/issues/73): "Avoid toString round-tripping in the collection multiplicity fallback" |
...
All deferrals from §11 were filed as GitHub issues in `vertiquehq/vertique-dev` on 2026-07-29, before
merge, per handoff ruling 5. Eighteen issues, **#59–#76**:

| # | Item |
|---|---|
| [#59](https://github.com/vertiquehq/vertique-dev/issues/59) | Collection-typed `@BeanParam` fields (incl. the top-level-vs-bean-field collision) |
| [#60](https://github.com/vertiquehq/vertique-dev/issues/60) | Whether multi-value ordering is a framework guarantee |
| [#61](https://github.com/vertiquehq/vertique-dev/issues/61) | Whether non-`List` multipart collection shapes become supported |
| [#62](https://github.com/vertiquehq/vertique-dev/issues/62) | BODY `componentType` divergence + the runtime-side proof residual |
| [#63](https://github.com/vertiquehq/vertique-dev/issues/63) | Classloader precedence in generated `loadClass` |
| [#64](https://github.com/vertiquehq/vertique-dev/issues/64) | HTTP-transport e2e for generated dispatch |
| [#65](https://github.com/vertiquehq/vertique-dev/issues/65) | Deduplicate the `List`-only native multipart rule |
| [#66](https://github.com/vertiquehq/vertique-dev/issues/66) | Null-returning `ParamConverter` for a `@DefaultValue` element |
| [#67](https://github.com/vertiquehq/vertique-dev/issues/67) | Non-`String` scalars skip the input-policy chain |
| [#68](https://github.com/vertiquehq/vertique-dev/issues/68) | Whether `@PathParam` collection shapes become supported |
| [#69](https://github.com/vertiquehq/vertique-dev/issues/69) | Compile-time mirrors for the shape rules |
| [#70](https://github.com/vertiquehq/vertique-dev/issues/70) | `ParamConverterNotFoundException` leaks internal type names (risk-accepted) |
| [#71](https://github.com/vertiquehq/vertique-dev/issues/71) | Inner-class-of-parameterized-owner element-type divergence |
| [#72](https://github.com/vertiquehq/vertique-dev/issues/72) | RFC 6265 cookie case-sensitivity |
| [#73](https://github.com/vertiquehq/vertique-dev/issues/73) | `toString` round-tripping in the multiplicity fallback |
| [#74](https://github.com/vertiquehq/vertique-dev/issues/74) | Whether to reject a non-self-comparable `SortedSet` element at startup |
| [#75](https://github.com/vertiquehq/vertique-dev/issues/75) | A test pinning the sorted-shape mount-then-500 contract |
| [#76](https://github.com/vertiquehq/vertique-dev/issues/76) | ADR references forbidden in packaged module docs leave some ADRs unreachable |

Legacy tracker hygiene stays post-merge per handoff ruling 2: close legacy #153 and #155 with pointers
once this merges.
