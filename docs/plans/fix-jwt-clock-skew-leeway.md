...
| A3 | 2026-07-30 | Proof-strength gap found in the S3/S4 simplify pass | §6 S3's `shouldRetainDefaultLeewayAfterRefreshTick` was specified to gate on the WireMock **request count** reaching 2. Three lanes independently observed that this proves a *fetch* occurred but not that the refreshed delegate was used — with the served key unchanged, the assertion is equally satisfiable by the pre-swap delegate. The test now rotates to `SECRET_B` before asserting, matching S4's sibling, so only a post-swap delegate can satisfy it. Strictly strengthens what the slice proves; no contract, scope, or consumer-visible behavior change, so no sign-off required. Re-proven able to fail: with `defaultValidation()` at `clockSkewSeconds(0)` it errors on the expired token — *after* the swap gate landed — and is the only failure in the class. |
| A4 | 2026-07-30 | Adjacent-defect rule, same execution path | Three pre-existing `RefreshableJwtAuthIT` tests waited on fixed 300–500 ms sleeps against a 200 ms refresh interval (~2.5 ticks of margin), the flake pattern `testing.md` § Async / Network IT Determinism names explicitly. S3/S4 introduced the deterministic gates that replace them, so they were converted in the same branch. The `setTimer(500, …)` proving the *absence* of requests after `close()` is deliberately retained — no gate can shortcut an absence proof. |
| A5 | 2026-07-30 | Coverage hole found reviewing S5 | §6 S5 specified the divergence check as "invoked from `JwtAuthModule.jwtBearerSchemeHandler`". That binding is only resolved by applications serving OpenAPI routes; one authenticating solely over a non-OpenAPI transport (WebSocket) resolves `RouteAuthHandler` instead and would skip the fail-fast entirely. Both providers now route through one `verifiedSchemeHandler` seam that runs the check. Idempotent, so an application resolving both simply runs it twice. Widens where an existing check fires; no contract or consumer-visible behavior change. **Residual, accepted:** an application that binds `JWTAuth` but resolves *neither* multibinding still never runs the check — there is no earlier startup seam that sees both the `JWTAuth` and the effective config. |
| A6 | 2026-07-30 | Proof gap found in the S5/S6 simplify pass | Nothing asserted that a `JwtAuthFactory` return is actually attested — removing the wrapper from either construction site left the whole suite green, because the example-app ITs exercise only the matching case, which passes identically with no attestation at all. `JwtAuthFactoryAttestationTest` now covers all ten construction paths and was proven to fail on exactly the four paths fed by whichever wrapper is removed. Strengthens proof of an existing invariant; no design change. |
