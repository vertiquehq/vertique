...
- ADR-0049: Workflow events as a separate module — event publication is opt-in and isolated from the persistence module.
- ADR-0050: Event envelope contract v1 — external event-envelope contract that external consumers depend on.
- ADR-0051: Optional intent kinds — `WORKFLOW_EVENT` may be opt-in and no-op when unused.
- ADR-0052: Reminders are event-only — task reminders are surfaced exclusively through workflow events.
